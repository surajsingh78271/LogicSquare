...
const findCaliforniaCafes = (sting) => {
    const cafes = [{name: "Bazaar Cafe",place_id: "kjk234g4gcvfx8usg1l33pi"},
  ...
  for (let i = 0; i < cafes.length; i++) {
    var { name, place_id } = cafes[i];
    if (name.includes(sting)) {
      for (let j = 0; j < places.length; j++) {
        if (places[j].id === place_id) {
          let { street_no, locality, postal_code, lat, long } = places[j];
          const temp = { name, street_no, locality, postal_code, lat, long };
...
const result = findCaliforniaCafes("Avenue");
